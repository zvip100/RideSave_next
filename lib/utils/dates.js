...
export function toNewYorkLocalTimestamp(timeStr, dateStr = null) {
  ...
  const parsed = dayjs.tz(`${dateStr} ${timeStr}`, 'YYYY-MM-DD h:mm a', 'America/New_York');
...
/**
 * Get the Sunday-Friday boundaries for a week at a given offset from the current week.
 * @param {number} weekOffset - 0 = current week, -1 = last week, -2 = two weeks ago, etc.
 * @returns {object} { startDate, endDate, label }
 */
export function getWeekBoundaries(weekOffset = 0) {
  const today = dayjs();
  // Current week's Sunday: day(0) gives Sunday of the current week
  const currentSunday = today.day(0);
  // Shift by weekOffset weeks
  const sunday = currentSunday.add(weekOffset, 'week');
  const friday = sunday.add(5, 'day');
  ...
  if (weekOffset === 0) {
    ...
    label = `${sunday.format('MMM D')} – ${friday.format('MMM D')}`;
  }
