...
const tripsModel = {
  ...
  async updateTrip(id, tripDetails) {
    try {
      const [trip] = await db.update(trips).set(tripDetails).where(eq(trips.id, id)).returning({ id: trips.id });
  ...
  async countTrips(userId) {
    try {
      const [result] = await db
        .select({
          total: count(),
          medicaid: sql`count(*) filter (where ${trips.type} = 'Medicaid')`,
        })
        .from(trips)
      ...
      return {
        total: result.total,
        medicaid: result.medicaid,
        cash: result.total - result.medicaid,
      };
    } catch (error) {
      console.error('Error counting trips:', error.message);
      throw error;
    }
  },
};
